...
\\\\
From the starting state, a string $w$ may only progress to the next state, $\{1, 2, 3\}$, by reading an $o$ character. It may only remain at $\{1, 2, 3\}$ by continuing to read $o$ inputs. A string may only progress to the third state, $\{1, 3\}$ when the machine then reads an $x$ character after the current $o$. An $o$ read subsequently at $\{1, 3\}$ returns it to the previous state, $\{1, 2, 3\}$.
...
In turn, any string $w$ finishing at $\{1, 2, 3\}$ must end in an $-o$ and any string finishing  at $\{1, 3\}$ must end in $-ox$. Thus, $w$ belongs to $L$.
