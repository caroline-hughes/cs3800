...
\newcommand{\yourcollaborators}{Ameya Bhat, Cory Langenbach}
...
\mbox { transitions}
\end{equation}
In an NFA, each state can have as many transitions as states, for each possible input as well as the 'free' $\varepsilon$ transition.  Thus, in an NFA with the max number of transitions, each state $q$ in $Q$ would have $\mid \Sigma \mid \times \mid Q \mid$ transitions (every input mapped to every state including itself) plus $Q-1$ more $\varepsilon$ transitions (to every state other than itself).
\end{enumerate}
...
\solution
\\\\
Let $w$ be any word accepted by the DFA above. 
\\\\
From the starting state, a string $w$ may only progress to the next state, $\{1, 2, 3\}$, by reading an $o$ character. It may only remain at $\{1, 2, 3\}$ by continuing to read $o$ inputs.
A string may only progress to the third state, $\{1, 3\}$ when the machine then reads an $x$ character after reading an $o$. An $o$ read subsequently at $\{1, 3\}$ returns it to the previous state, $\{1, 2, 3\}$.
\\\\
From $\{1, 2, 3\}$, a character that is not $o$ or $x$ returns the machine to the initial rejection state. From $\{1, 3\}$, a character that is not $o$ returns the machine to the initial rejection state. 
\\\\
In turn, any string accepted by the machine $w$ must end in an $-o$ and finish at $\{1, 2, 3\}$ or must end in an $-ox$ and finish at $\{1, 3\}$. Since $L$ is the set of strings ending in $-o$ or $-ox$, any accepted string $w$ belongs to $L$.
...
\solution
\\\\
Let $w$ be any word from $L$. In other words, $w$ is a string which ends in either $-o$ or $-ox$.
\\\\
1) Say, as in the first case, that $w$ ends in $-o$. There are three possible states of the machine when reading the final $o$ character. If reading the final $o$ from state $1$, the string ends at the accepting state $\{1, 2, 3\}$. If reading the final $o$ from state $\{1, 2, 3\}$, the string ends at the accepting state $\{1, 2, 3\}$. If reading the final $o$ from state $\{1, 3\}$, the string ends at the accepting state $\{1, 2, 3\}$. Thus, any string $w$ which ends in $-o$ is accepted by the machine.
\\\\
2) Say, as in the other case, that $w$ ends in $-ox$. There are three possible states of the machine when reading the second to last $o$ character. If reading the $o$ from state $1$, we transition to state $\{1, 2, 3\}$. If reading the $o$ from state $\{1, 2, 3\}$, we stay at state $\{1, 2, 3\}$. If reading the $o$ from state $\{1, 3\}$, we transition to state $\{1, 2, 3\}$. Thus, when reading the final character, $x$, we may only be at  state $\{1, 2, 3\}$. When reading the final $x$ from state $\{1, 2, 3\}$, the string is accepted at state $\{1, 3\}$. Thus, any string $w$ which ends in $-ox$ is accepted by the machine.
\\\\
Thus, since a string $w$ from $L$ ends in either $-ox$ or $-o$, that string must be accepted by the machine.
