...
\mbox { transitions}
\end{equation}
In a DFA, the max number of transitions is the total number of characters in the alphabet times the total number of states.
...
\begin{equation}
\mid \Sigma_e \mid ^{\mid Q \mid}
\mbox { transitions}
\end{equation}
In an NFA, the max number of transitions is the total number of characters in the alphabet including the 'free' $\varepsilon$, to the power of the total number of states.
...
\node[state, accepting, right of=q2] (q3) {$q_3$};
\draw (q1) edge[loop above] node{a-n, p-z} (q1)
(q1) edge[bend left, above] node{o} (q2)
(q2) edge[bend left, below] node{a-n, p-w, y-z} (q1)
(q2) edge[above] node{x, o} (q3)
\end{tikzpicture}
